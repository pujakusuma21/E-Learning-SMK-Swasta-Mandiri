...
class Pengajar extends MY_Controller
{
    ...
    function absensi()
    {
        ...
        $this->twig->display('absensi.html', $data);
    }

    function detail_absensi($pengajar_id, $mapel_id, $kelas_id)
    {
        if (!is_pengajar()) {
            redirect('pengajar');
        }
        ...
        $data['absensi']       = $this->absensi_model->retrieve_all($mapel_id,$kelas_id);
        $data['pengajar_login'] = $this->login_model->retrieve(get_sess_data('login', 'id'));
        $data['status_id']      = get_sess_data('user', 'status_id');

        $data['url_param'] = $pengajar_id.'/'.$mapel_id.'/'.$kelas_id;
